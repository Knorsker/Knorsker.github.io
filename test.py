...
# import cph map
with urlopen(
        'https://wfs-kbhkort.kk.dk/k101/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=k101:bydel&outputFormat=json&SRSNAME=EPSG:4326') as response:
    cph = json.load(response)

# import bikeracks
with urlopen(
        'https://wfs-kbhkort.kk.dk/k101/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=k101:cykelstativ&outputFormat=json&SRSNAME=EPSG:4326&CQL_Filter=stativ_type%3C%3E%27Bycykelstativ%27') as response:
    bikeracks = json.load(response)

# import bikelanes
with urlopen(
        'https://wfs-kbhkort.kk.dk/k101/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=k101:cykeldata_kk&outputFormat=json&SRSNAME=EPSG:4326') as response:
...
# add column with area of each district
...
# make list of x and y for each feature in bikeracks
lats = []
longs = []
category = []
size = []
...
def display_choropleth(unit):
    ...
    # Create a new figure with the updated color unit
    fig = px.choropleth_mapbox(district_data,
                               geojson=cph,
                               color=unit,
                               locations="bydel",
                               featureidkey="properties.navn",
                               range_color=(0, district_data[unit].max()),
                               center={"lat": 55.676098, "lon": 12.568337},
                               zoom=10.5,
                               color_continuous_scale="Viridis",
                               mapbox_style="carto-positron", opacity=0.6)

    # Update layout inside the callback function
    fig.update_layout(
